...
type ReportData = Report & { filesAffected?: number; repoFullName?: string }
...
export function HealthScoreHero({ report }: HealthScoreHeroProps) {
  ...
  const handleExportJSON = () => {
    ...
    const fileName = report.repoFullName ? report.repoFullName.replace('/', '-') : 'repository'
    downloadNode.setAttribute("download", `gitpulse-report-${fileName}.json`)
    ...
    if (navigator.share) {
      try {
        await navigator.share({
          ...
          text: `Check out this code analysis report for ${report.repoFullName || 'this repository'}`,
          url: shareUrl,
        })
      } catch (err) {
